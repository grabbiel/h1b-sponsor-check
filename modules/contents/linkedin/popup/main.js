class PopUpMananger{
    ...
    #initial_fetch = async(companyname, jobstatus, h1bdata) =>{
        if(document.getElementById("popup-container")){
            ...
            await fetch(chrome.runtime.getURL("modules/popup/container.html")).then(
    ...
    #get_close_button = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/closeBtn.html")).then(
    ...
    #get_return_button = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/returnBtn.html")).then((html)=>html.text()).then((txt)=>{
    ...
    #get_donate_button = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/donateBtn.html")).then(
            (html)=>html.text()).then((txt)=>{
                ...
                this.donatebtn.addEventListener("click",e=>{
                    ...
                    this.#set_leaving_page();
                });
        });
    }
    #get_feedback_button = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/feedbackBtn.html")).then(
            (html)=>html.text()).then((txt)=>{
    ...
    #insert_main_text = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/text/summary.html")).then(
            (html)=>html.text()).then((txt)=>{
    ...
    #set_leaving_page = async() =>{
        this.closebtn.remove();
        this.feedbackbtn.remove();
        this.donatebtn.remove();
        this.summarymode.querySelector("p").innerText = "you are about to leave this page";
        this.#get_leave_button().then(()=>{
            this.#get_refuse_button();
        });
        
    }
    #get_leave_button = async(/*donate?go:feedback*/) =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/leaveBtn.html")).then(
            (html)=>html.text()).then((txt)=>{
                this.bottombtncontainer.firstElementChild.insertAdjacentHTML('beforebegin',txt);
                this.leavebtn = document.getElementById("popup-leave-btn");
                this.leavebtn.addEventListener("click",e=>{
                    console.log("you are leaving this site...");
                });
        });
    }
    #get_refuse_button = async() =>{
        await fetch(chrome.runtime.getURL("modules/popup/buttons/refuseBtn.html")).then(
            (html)=>html.text()).then((txt)=>{
                this.bottombtncontainer.firstElementChild.insertAdjacentHTML('afterend',txt);
